...
import { downloadAudio } from "./utils/functions/download-audio";
    ...
    <main
      className={`min-h-dvh grid place-items-center max-sm:px-8
                py-12 transition-all duration-300 ease-in-out ${
                  !details && "max-sm:pb-40 sm:pb-52 md:pb-64 lg:pb-72"
                }`}
    >
      <div className="w-full max-w-xl flex flex-col items-center gap-8 relative pb-8">
        {/* logo */}
        <img
          className={`w-52 animate-glow-pulse mb-8 max-sm:hidden ${
            details && "hidden"
          }`}
          src="./icon.svg"
          alt="Logo de MOPI"
        ...
        {details && (data.id_youtube || data.url_framely) && (
          <div className="w-full flex flex-col gap-8 animate-fade-in">
            {/* titulo y bitrate */}
            <div className="w-full flex gap-1.5 max-sm:flex-col max-sm:gap-8">
              {/* input-title */}
              <div className="input_div">
            ...
            <span className="absolute -bottom-12 sm:-bottom-9 font-normal text-red-400">
              {data.error}
            </span>
          </div>
        )}
